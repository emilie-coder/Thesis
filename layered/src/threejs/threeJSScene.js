...
const handleDrag = (event) => {
  ...
  // Round the current position to the nearest multiple of 5
  const roundedX = Math.round(objectToUpdate.position.x / 5) * 5;
  const roundedY = Math.round(objectToUpdate.position.y / 5) * 5;
  const roundedZ = Math.round(objectToUpdate.position.z / 5) * 5;

  const newPosition = { x: roundedX, y: roundedY, z: roundedZ };
